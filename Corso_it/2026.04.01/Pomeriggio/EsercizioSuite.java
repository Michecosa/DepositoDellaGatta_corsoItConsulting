...
public class EsercizioSuite {
    public static void main(String[] args) {
        ...
        do {
            System.out.println("\n--- MENU SARTORIA ---");
            System.out.println("1. Aggiungi capo principale");
            System.out.println("2. Aggiungi componente finitura");
            System.out.println("3. Mostra capi principali");
            System.out.println("4. Mostra componenti finitura");
            ...
            switch (scelta) {
                case 1 -> aggiungiCapo(sartoria, sc);
                case 2 -> aggiungiAccessorio(sartoria, sc);
                case 3 -> sartoria.mostraCapi();
                case 4 -> sartoria.mostraAccessori();
                case 5 -> sartoria.mostraRiepilogo();
        ...
        try {
            switch (tipo) {
                case 1 -> {
                    ...
                    int bottoni = Integer.parseInt(sc.nextLine().trim());
                    sartoria.aggiungiCapo(new Giacca(codice, nome, tessuto, colore, taglia, prezzo, bottoni));
                }
                case 2 -> {
                    System.out.print("Tipo taglio: ");
                    String taglio = sc.nextLine();
                    sartoria.aggiungiCapo(new Pantalone(codice, nome, tessuto, colore, taglia, prezzo, taglio));
                }
                case 3 -> {
                    System.out.print("Rever presente? (s/n): ");
                    boolean rever = sc.nextLine().trim().equalsIgnoreCase("s");
                    sartoria.aggiungiCapo(new Gilet(codice, nome, tessuto, colore, taglia, prezzo, rever));
                }
                default -> { System.out.println("Tipo non valido, capo non aggiunto"); return; }
            }
        ...
        try {
            switch (tipo) {
                case 1 -> {
                    ...
                    double larghezza = Double.parseDouble(sc.nextLine().trim());
                    sartoria.aggiungiAccessorio(new Cravatta(codice, nome, materiale, colore, prezzo, larghezza));
                }
                case 2 -> {
                    System.out.print("Tipo chiusura: ");
                    String chiusura = sc.nextLine();
                    sartoria.aggiungiAccessorio(new Papillon(codice, nome, materiale, colore, prezzo, chiusura));
                }
                case 3 -> {
                    System.out.print("Piega decorativa: ");
                    String piega = sc.nextLine();
                    sartoria.aggiungiAccessorio(new Pochette(codice, nome, materiale, colore, prezzo, piega));
                }
                default -> { 
                    System.out.println("Tipo non valido, accessorio non aggiunto"); 
                    return; 
                }
            }
            System.out.println("Componente aggiunto con successo");
        } catch (IllegalArgumentException e) {
            System.out.println("Errore: " + e.getMessage());
        }
    }
